...
class FrameTrayecto(SectionFrameBase):
    def __init__(self, master,controlador,vcmd_num, vcmd_fecha,titulo = "Trayecto"):
        ...
        self.botones_tramos_frame.pack(pady=(20, 0), anchor="w")
    ...
    def agregar_tramos(self):
        ...
        if self.lista_datos_tramos:
            for i, datos in enumerate(self.lista_datos_tramos):
                ...
                for entry, campo in zip(tramo.entries_a_validar, campos):
                    ...
                    # CORRECCIÓN: Convertir el valor a string y manejar valores None
                    if valor is None:
                        valor = ""
                    else:
                        valor = str(valor)
                    ...
                    if isinstance(entry, ctk.CTkOptionMenu):
                        ...
                        # Limpiar el campo antes de insertar
                        entry.delete(0, 'end')
        ...
        for i in range(numero_tramos):
            tramo = FrameTramos(scrollable_frame, self.controlador, self.vcmd_num, self.vcmd_fecha, titulo=f"Tramo #{i+1}")
            tramo.numero_entry.insert(0, str(i + 1))  # Convertir a string
            tramo.numero_entry.configure(state="disabled")
            tramo.pack(fill="x", padx=10, pady=10)
            self.lista_tramos.append(tramo)
    ...
    def agregar_nuevos_tramos(self):
        ...
        if cantidad_tramos_nuevos > 0:
            for i in range(cantidad_tramos_nuevos):
                ...
                frame.numero_entry.insert(0, str(cantidad_tramos_actuales + i + 1))  # Convertir a string
                frame.numero_entry.configure(state="disabled")
    ...
    def set_datos(self, datos):
        if datos:
            ...
            def safe_option_menu_set(option_menu, value, default_value=""):
                if value is None:
                    option_menu.set(default_value)
                    return
                
                try:
                    available_values = option_menu.cget("values")
                    if str(value) in available_values:
                        option_menu.set(str(value))
                    else:
                        current_values = list(available_values)
                        if str(value) not in current_values:
                            current_values.append(str(value))
                            option_menu.configure(values=current_values)
                        option_menu.set(str(value))
                except Exception:
                    option_menu.set(str(value) if value is not None else default_value)
            
            # Campos de texto normales
            self.numero_entry.insert(0, safe_value(datos.get("numero")))
            self.numero_entry.configure(state="disabled")
            
            self.nombre_entry.insert(0, safe_value(datos.get("nombre")))
            self.nombre_entry.configure(state="disabled")
            
            self.duracion_semanas_entry.insert(0, safe_value(datos.get("duracion_semanas")))
            self.duracion_semanas_entry.configure(state="disabled")
            
            self.duracion_horas_entry.insert(0, safe_value(datos.get("duracion_horas")))
            self.duracion_horas_entry.configure(state="disabled")
            
            self.creditos_minimos_entry.insert(0, safe_value(datos.get("creditos_minimos")))
            self.creditos_minimos_entry.configure(state="disabled")
            
            self.creditos_maximos_entry.insert(0, safe_value(datos.get("creditos_maximos")))
            self.creditos_maximos_entry.configure(state="disabled")
            
            self.objetivos_entry.insert(0, safe_value(datos.get("objetivos")))
            self.objetivos_entry.configure(state="disabled")
            
            # OptionMenus
            safe_option_menu_set(self.numero_tramos_menu, datos.get("numero_tramos"), "1")
            self.numero_tramos_menu.configure(state="disabled")
            
            safe_option_menu_set(self.perfil_egreso_menu, datos.get("perfil_egreso"), "Técnico Superior Universitario")
            self.perfil_egreso_menu.configure(state="disabled")
            
            # Manejo especial para campos booleanos
            obligatorio_value = datos.get("obligatorio")
            if obligatorio_value is None:
                self.obligatorio_menu.set("Si")
            else:
                self.obligatorio_menu.set("Si" if obligatorio_value else "No")
            self.obligatorio_menu.configure(state="disabled")
            
            secuencial_value = datos.get("secuencial")
            if secuencial_value is None:
                self.secuencial_menu.set("Si")
            else:
                self.secuencial_menu.set("Si" if secuencial_value else "No")
            self.secuencial_menu.configure(state="disabled")
            
            safe_option_menu_set(self.estado_option_menu, datos.get("estado"), "activo")
            self.estado_option_menu.configure(state="disabled")
            
            # Manejo de tramos
            if datos.get("lista_tramos"):
