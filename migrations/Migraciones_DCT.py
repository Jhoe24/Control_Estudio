"""
Script para migrar datos de la tabla DCT desde archivos Excel a una nueva estructura SQLite3.

Este script:
1. Permite seleccionar y cargar datos de archivos Excel que contienen la tabla DCT
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"logg/migracion_dct_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
        logging.StreamHandler(sys.stdout)
    ]
    
)
logger = logging.getLogger("migracion_dct")
...
class MigradorDCT:
    ...
    def seleccionar_archivo_excel(self) -> str:
        ...
        file_path = filedialog.askopenfilename(
            title="Seleccione el archivo Excel con los datos de DCT",
    ...
    def cargar_datos_excel(self, file_path: str) -> bool:
        ...
        try:
            ...
            columnas_requeridas = [
                'D_AUXILIAR', 'D_ID', 'D_ABREVIA', 'D_NOMBRE', 'D_SEXO',
                'D_FNACI', 'D_DIRECC', 'D_TEL1', 'D_TEL2', 'A_CORREO'
            ]
    ...
    def migrar_informacion_personal(self) -> bool:
        """
        Migra la información personal de los docentes a la tabla informacion_personal.
        ...
        try:
            ...
            for index, row in self.df_alm.iterrows():
                ...
                # Formatear datos
                nombres = str(row['D_NOMBRE']).strip()
                ...
                direccion = str(row.get('D_DIRECC', '')).strip()
                if '@' in direccion:
                    correo = direccion
                ...
                if resultado:
                    logger.warning(f"docente con documento {doc_id} ya existe, se actualizará")
                    
                    # Actualizar información existente
                    self.cursor.execute("""
                        UPDATE informacion_personal 
                        SET nombres = ?, fecha_nacimiento = ?, sexo = ?, 
                            correo_electronico = ?
                    ...
                    self.cursor.execute("""
                        INSERT INTO informacion_personal (
                            documento_identidad, tipo_documento, nombre, 
                            fecha_nacimiento, sexo, estado_civil, nacionalidad, 
                            lugar_nacimiento, correo_electronico, fecha_registro, tipo
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
                        doc_id, 'cedula', nombres, 
                        fecha_nacimiento, sexo, 'Soltero', 'Venezolano', 
                        None, correo, None, 'docente'
                    ))
    ...
    def migrar_telefonos(self) -> bool:
        ...
        try:
            ...
            for index, row in self.df_alm.iterrows():
                ...
                # Procesar teléfonos
                for i, tel_campo in enumerate(['D_TEL1', 'D_TEL2']):
            ...
            for index, row in self.df_alm.iterrows():
                ...
                if direccion_completa and '@' not in direccion_completa:
                    ...
                    if dir_existente:
                        ...
                        self.cursor.execute("""
                            UPDATE direcciones 
                            SET estado = ?, direccion_completa = ?
                            WHERE persona_id = ?
                        """, (
                            'BARINAS', direccion_completa, persona_id
                        ))
                    else:
    ...
    def migrar_docentes(self) -> bool:
        """
        Migra la información  de los docentes a la tabla docentes.
        
        Returns:
            bool: True si la migración fue exitosa, False en caso contrario
        """
        try:
            logger.info("Iniciando migración de información de los docentes...")
            ...
            for index, row in self.df_alm.iterrows():
               ...
               abreviatua = str(row.get('D_ABREVIA', '')).strip()
               auxiliar = str(row.get('D_AUXILIAR', '')).strip()
...
def main():
    ...
    print("=" * 60)
    print("SISTEMA DE MIGRACIÓN DE DATOS DCT A NUEVA ESTRUCTURA SQLITE3")
    print("=" * 60)
    print("\nEste script migra los datos de la tabla DCT desde un archivo Excel")
    ...
    print(f"- Docentes: {conteo['docentes']}")
