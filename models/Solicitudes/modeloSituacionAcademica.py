...
class ModeloSituacionAcademica:
    ...
    def generar_pdf(self):
        """ Genera el archivo PDF con la situación académica. """
        # Aumentar topMargin para que quepa el encabezado de varias líneas sin sobreponerse al contenido
        doc = SimpleDocTemplate(str(self.filepath), pagesize=letter,
        ...
        # El estilo 'Titulo' se mantiene igual, pero se añade un 'spaceBefore' para moverlo hacia abajo
        style_titulo = ParagraphStyle(name='Titulo', parent=styles['h1'], fontName='Helvetica-Bold',
                                      fontSize=self._titulo['tamanoFuente'], alignment=TA_CENTER, spaceAfter=10, spaceBefore=0) # spaceBefore: 0, ya que el espacio lo da el margin.
        style_info = ParagraphStyle(name='Info', parent=styles['Normal'], fontName=self._fuente, fontSize=9, leading=12, alignment=TA_LEFT)
        style_firma = ParagraphStyle(name='Firma', parent=styles['Normal'], fontName=self._fuente,
                                      fontSize=self._firmaCoordinador['tamanoFuente'], alignment=TA_CENTER, leading=11, spaceBefore=40) # Aumentar spaceBefore
        ...
        # 2. Tabla de Datos del Estudiante (Multi-columna)
        data_info = []
        for row in self._datosPrimeraTabla:
            data_info.append([Paragraph(cell, style_info) for cell in row])
        
        tabla_info = Table(data_info, colWidths=[2.2*inch, 1.8*inch, 3.1*inch], hAlign='LEFT')
        ...
        for nota in self.listNotas:
            ...
            data_notas.append([
                ...
                trayecto_val, 
                Paragraph(nota.get('nombre_unidad_curricular', 'N/A'), style_info),
                f"{valor_nota:.2f}",
        ...
        # 4. Firma (Solo la línea y el nombre, la información adicional va en la cabecera de la página 2)
        # La firma de la página 1 requiere el triple <br/> para bajar el espacio de la línea de firma
        Story.append(Paragraph(self._firmaCoordinador['texto'].replace("<br/>","<br/><br/><br/>", 1), style_firma))

        # --- Construir el PDF ---
        try:
            if not os.path.exists(self._imgLogo):
                 print(f"Advertencia: No se encontró el logo en la ruta: {self._imgLogo}. El PDF se generará sin logo.")
            
            # ************ CORRECCIÓN DE onLaterPages ************
            # Si tiene más de una página, usa la cabecera secundaria y no usa pie de página.
            doc.build(Story, 
                      onFirstPage=self._cabecera_pie_pagina, 
                      onLaterPages=self._cabecera_paginas_secundarias)
            # ****************************************************
